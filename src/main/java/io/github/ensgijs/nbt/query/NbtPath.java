...
public class NbtPath {
    ...
    public static NbtPath of(String selector) {
        ...
        for (String part : parts) {
            if (part.isEmpty() && !allowEmpty)
                throw new IllegalArgumentException("empty name at: " + markLocation(selector, partPos, 0));
            ...
            if (bracketOpenIdx < 0 || bracketCloseIdx < bracketOpenIdx) {
                if (bracketCloseIdx >= 0)
                    throw new IllegalArgumentException("unexpected close bracket at: " + markLocation(selector, bracketCloseIdx + partPos, 1));
                name = part;
            } else {
                if (bracketCloseIdx < 0)
                    throw new IllegalArgumentException("unclose bracket at: " + markLocation(selector, bracketOpenIdx + partPos, 1));
    ...
    public <R> R get(Tag<?> root) {
        ...
        for (Evaluator evaluator : evalChain) {
            ...
            if (!(node instanceof Tag))
                throw new IllegalStateException("Expected TAG but was " + node.getClass().getTypeName() + "\n" + makeErrorHint(evaluator));
