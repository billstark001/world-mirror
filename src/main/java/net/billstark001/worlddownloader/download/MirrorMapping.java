 ...
 *       ({@link net.billstark001.worlddownloader.config.ModConfig.SaveLocation#DOWNLOADED}).</li>
 *   <li>{@code saves/} — the vanilla saves directory
 *       ({@link net.billstark001.worlddownloader.config.ModConfig.SaveLocation#SAVES}).</li>
 * </ul>
...
public class MirrorMapping {
     ...
     *                         ({@link net.billstark001.worlddownloader.config.ModConfig.SaveLocation#name()})
    ...
    public String getResolvedFolderName(String sourceId, String saveLocationName, Path base) {
        ...
        if (!candidate.toFile().exists()) {
            WDLogger.debug("Resolved folder no longer exists, invalidating: " + candidate);
            resolvedFolderNames.remove(key);
            save();
            return null;
        }
        if (!WorldMetadata.isOwnedBy(candidate, sourceId)) {
            WDLogger.debug("Resolved folder ownership mismatch, invalidating: " + candidate);
            resolvedFolderNames.remove(key);
            save();
            return null;
        }
