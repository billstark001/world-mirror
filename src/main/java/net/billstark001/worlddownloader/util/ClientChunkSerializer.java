...
public class ClientChunkSerializer {
  public static NbtCompound serialize(ClientWorld world, Chunk chunk) {
    ...
    chunkNbt.putInt("DataVersion", SharedConstants.getGameVersion().comp_4026().comp_4038());
    ...
    Registry<ChunkStatus> chunkStatusRegistry = world.getRegistryManager().get(RegistryKeys.field_41261);
    ...
    for (Map.Entry<Heightmap.Type, Heightmap> entry : (Iterable<Map.Entry<Heightmap.Type, Heightmap>>)chunk.getHeightmaps()) {
      heightmaps.put(((Heightmap.Type)entry.getKey()).getId(), (NbtElement)new NbtLongArray(((Heightmap)entry.getValue()).asLongArray()));
    }
    chunkNbt.put("Heightmaps", (NbtElement)heightmaps);
    ...
    for (int y = minSectionY; y < world.getTopSectionCoord(); y++) {
      ...
      if (section != null && !section.isEmpty()) {
        ...
        sectionNbt.put("block_states", (NbtElement)serializeBlockStates(blockContainer, (RegistryWrapper.WrapperLookup)world.getRegistryManager()));

        
        ReadableContainer<RegistryEntry<Biome>> biomeContainer = section.getBiomeContainer();
        sectionNbt.put("biomes", (NbtElement)serializeBiomes(biomeContainer, biomeRegistry));
        
        sections.add(sectionNbt);
      } 
    } 
    chunkNbt.put("sections", (NbtElement)sections);
    ...
    for (BlockPos pos : chunk.getBlockEntityPositions()) {
      ...
      if (be != null) {
        try {
          NbtCompound beNbt = serializeBlockEntityWithItems(be, (RegistryWrapper.WrapperLookup)world.getRegistryManager());
          if (beNbt != null) {
    ...
    chunkNbt.put("block_entities", (NbtElement)blockEntities);

    
    chunkNbt.put("structures", (NbtElement)new NbtCompound());
    ...
    try {
      ...
      try {
        ...
        if (client.world != null) {
          ...
          Registry<BlockEntityType<?>> blockEntityTypeRegistry = client.world.getRegistryManager().get(RegistryKeys.field_41255);
      ...
      try {
        ...
        if (additionalData.contains("Items") && !beNbt.contains("Items")) {
          ...
          System.out.println("✅ Saved items for " + blockEntity.getType().toString() + " at " + String.valueOf(blockEntity.getPos()));
        } 
        ...
        if (blockEntity instanceof Nameable) { Nameable nameable = (Nameable)blockEntity;
  ...
  private static NbtCompound serializeBlockStates(PalettedContainer<BlockState> container, RegistryWrapper.WrapperLookup registryLookup) {
    ...
    Registry<Block> blockRegistry = (Registry<Block>)registryLookup.getWrapperOrThrow(RegistryKeys.field_41254);
    ...
    for (int y = 0; y < 16; y++) {
      for (int z = 0; z < 16; z++) {
        for (int x = 0; x < 16; x++) {
          int paletteIndex; BlockState state = (BlockState)container.get(x, y, z);
          
          if (!paletteIndices.containsKey(state)) {
            paletteIndex = palette.size();
            palette.add(state);
            paletteIndices.put(state, Integer.valueOf(paletteIndex));
          } else {
            paletteIndex = ((Integer)paletteIndices.get(state)).intValue();
          } 
          rawData.add(Integer.valueOf(paletteIndex));
        } 
      } 
    } 
    ...
    for (BlockState state : palette) {
      ...
      if (!state.getEntries().isEmpty()) {
        ...
        stateNbt.put("Properties", (NbtElement)properties);
      } 
      paletteNbt.add(stateNbt);
    } 
    blockStatesNbt.put("palette", (NbtElement)paletteNbt);
  ...
  private static NbtCompound serializeBiomes(ReadableContainer<RegistryEntry<Biome>> container, Registry<Biome> biomeRegistry) {
    ...
    for (int y = 0; y < 4; y++) {
      for (int z = 0; z < 4; z++) {
        for (int x = 0; x < 4; x++) {
          int paletteIndex; RegistryEntry<Biome> biome = (RegistryEntry<Biome>)container.get(x, y, z);
          
          if (!paletteIndices.containsKey(biome)) {
            paletteIndex = palette.size();
            palette.add(biome);
            paletteIndices.put(biome, Integer.valueOf(paletteIndex));
          } else {
            paletteIndex = ((Integer)paletteIndices.get(biome)).intValue();
          } 
          rawData.add(Integer.valueOf(paletteIndex));
        } 
      } 
    ...
    biomesNbt.put("palette", (NbtElement)paletteNbt);
  ...
  private static long[] packIntoLongArray(List<Integer> data, int bitsPerEntry) {
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      result[arrayIndex] = result[arrayIndex] | (((Integer)data.get(i)).intValue() & mask) << bitIndex;
    } 
