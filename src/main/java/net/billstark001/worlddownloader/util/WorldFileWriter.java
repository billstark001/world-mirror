...
public class WorldFileWriter {
  public static void writeLevelDat(Path worldFolder) {
    try {
      ...
      posList.add(NbtDouble.of(0.0D));
      player.put("Pos", (NbtElement)posList);

      
      NbtList rotationList = new NbtList();
      rotationList.add(NbtDouble.of(0.0D));
      rotationList.add(NbtDouble.of(0.0D));
      player.put("Rotation", (NbtElement)rotationList);
      data.put("Player", (NbtElement)player);
      
      dataWrapper.put("Data", (NbtElement)data);
      
      File levelDat = worldFolder.resolve("level.dat").toFile();
      OutputStream out = new FileOutputStream(levelDat); 
      try { NbtIo.writeCompressed(dataWrapper, out);
        out.close(); } catch (Throwable throwable) { try { out.close(); } catch (Throwable throwable1) { throwable.addSuppressed(throwable1); }
         throw throwable; }
