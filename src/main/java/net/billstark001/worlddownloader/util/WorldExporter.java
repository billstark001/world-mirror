...
public class WorldExporter {
  public static void createLoadableWorld(File worldFolder) {
    try {
      ...
      try { out.writeLong(System.currentTimeMillis());
        out.close(); } catch (Throwable throwable) { try { out.close(); }
        catch (Throwable throwable1) { throwable.addSuppressed(throwable1); }
         throw throwable; }
       NbtCompound root = new NbtCompound();
      root.putInt("DataVersion", SharedConstants.getGameVersion().comp_4026().comp_4038());
      ...
      data.put("WorldBorder", (NbtElement)worldBorder);
      ...
      data.put("GameRules", (NbtElement)gameRules);
      ...
      pos.add(NbtDouble.of(0.0D));
      player.put("Pos", (NbtElement)pos);

      
      NbtList rotation = new NbtList();
      rotation.add(NbtFloat.of(0.0F));
      rotation.add(NbtFloat.of(0.0F));
      player.put("Rotation", (NbtElement)rotation);
      ...
      motion.add(NbtDouble.of(0.0D));
      motion.add(NbtDouble.of(0.0D));
      player.put("Motion", (NbtElement)motion);
      ...
      player.put("Inventory", (NbtElement)new NbtList());
      player.put("EnderItems", (NbtElement)new NbtList());
      
      data.put("Player", (NbtElement)player);

      
      root.put("Data", (NbtElement)data);

      
      File levelDat = new File(worldFolder, "level.dat");
      FileOutputStream fos = new FileOutputStream(levelDat); 
      try { NbtIo.writeCompressed(root, fos);
        fos.close(); } catch (Throwable throwable) { try { fos.close(); } catch (Throwable throwable1) { throwable.addSuppressed(throwable1); }
         throw throwable; }
